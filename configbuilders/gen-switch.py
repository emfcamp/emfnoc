...
def generate(override_template):
    if (
        os.path.isfile("data/switches")
        and os.path.isfile("data/users")
        and os.path.isfile("data/addressing")
    ):
        ...
        for sw in switches:
            if "Camper-VLAN" in sw:
                ...
                if sw["Camper-VLAN"] in cvlan_to_sw:
                    print(
                        "PANIC: Duplicate camper vlan id: >" + sw["Camper-VLAN"] + "<"
                    )
                    print(
                        "for "
                        + sw["Hostname"]
                        + " and "
                        + cvlan_to_sw[sw["Camper-VLAN"]]
                    )
                    sys.exit(1)
        ...
        for link in links:
            ...
            if not switch_hostname_exists(switches, link["Switch1"]):
                print(
                    "WARNING: switch in links but not on the Switches sheet >"
                    + link["Switch1"]
                    + "<"
                )
        ...
        # all sitewide vlans
        sitewide_vlans = ""
        for id, vlan in list(vlans.items()):
            if vlan["sitewide"]:
                if sitewide_vlans:
                    sitewide_vlans += ","
    ...
    for sw in sw_uplink_vlans:
        ks = list(sw_uplink_vlans[sw].keys())
        ks.sort()
        o[sw] = ks
    ...
    for sw in switches:
        ...
        for i in range(int(sw["Camper-Ports"])):
            sw["port_configs"].append(
                {"description": "Camper", "vlan": sw["Camper-VLAN"]}
            )
        ...
        for port_type in reversed(port_types):
            if ("#" + port_type["Port-Type"]) in sw:
                for i in range(int(sw["#" + port_type["Port-Type"]])):
                    sw["port_configs"].append(
                        {
                            "description": port_type["Port-Type"],
                            "vlan": port_type["VLAN"],
                        }
                    )
        print(sw)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate cisco IOS config files from gdocs."
    )
    ...
    parser.add_argument("--generate", action="store_true", help="generate the configs")

    parser.add_argument("--template", action="store", help="override the template")
